    ...
    if (program.config) {
      ...
      tm.lodeConfig()
        ...
        .then(() => {
          ...
          console.log(symbols.success, chalk.blue('打卡时间:' + dateToFlag(workTime, true)));
          console.log(symbols.warning, chalk.red('下班时间:' + dateToFlag(leaveTime, true)));
          console.log(symbols.info, chalk.yellow('当前时间:' + dateToFlag(new Date(), true)));
          console.log(symbols.info, chalk.yellow('剩余时间:' + timeformat(leaveTime)));
...
function dateToFlag(date: Date, showMore: boolean = false): string {
  return `${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()}${showMore ? `-${date.getHours()}:${date.getMinutes() == 0 ? '00' : date.getMinutes()}` : ''}`;
}
